...
@section('content')
    <div class="card">
        <h5 class="card-header">Edit Sales Representative</h5>
        <div class="card-body">
            <form action="{{route('salesrep.update',$salesRep['id'])}}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <select name="route_id" class="form-control">
                                    @foreach($routes as $r)
                                        <option
                                            {{($salesRep['route_id']===$r->id) ?  "selected": "" }} value="{{$r->id}}">{{$r->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
